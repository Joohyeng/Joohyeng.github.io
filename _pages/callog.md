...
### 3. Kafka·MongoDB 권한 동기화로 AI 판사 서버를 app 의존에서 분리

AI 판사 모듈을 처음 분리했을 때는 프론트엔드의 검수 요청이 기존 `app`을 거쳐 AI 판사 서버로 전달되도록 연결했습니다. 빠르게 기능을 분리해 동작을 확인할 수 있었지만, `app`이 요청 전달과 권한 정보 확인의 중간 경로로 남아 검수 서비스가 완전히 독립적이지 못했습니다. `app`의 지연이나 장애가 검수 요청에도 직접 영향을 줄 수 있다는 점이 과제였습니다.

<div class="port-decision">
최종적으로 캠페인이 생성되거나 참여자 권한이 변경될 때 Kafka 이벤트를 발행하고, AI 판사 서버가 이를 소비해 MongoDB에 캠페인·권한 정보를 저장하도록 구성했습니다. 검수 요청 경로도 AI 판사 서버로 직접 연결해, AI 판사 서버가 동기화된 권한 정보를 바탕으로 `app`을 거치지 않고 검수를 처리하도록 했습니다. 즉, 검수 요청과 권한 확인 흐름에서는 `app`의 동기 호출 의존성을 제거했습니다. 이 과정에서 MSA 분리는 서버를 나누는 작업만이 아니라, 필요한 데이터를 언제·어떤 방식으로 전달하고 정합성을 관리할지까지 함께 결정해야 한다는 점을 배웠습니다.
</div>
