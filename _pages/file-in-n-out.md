...
### 4. 채팅방 목록의 N+1 조회 병목을 페이지네이션·집계 쿼리·Redis로 개선

채팅방 목록에는 마지막 메시지, 읽지 않은 메시지 수, 참여자 수를 함께 표시해야 했습니다. 초기 구현에서는 채팅방을 조회한 뒤 각 채팅방의 부가 정보를 다시 조회하면서, 채팅방 수만큼 데이터베이스 접근이 반복되는 N+1 문제가 발생했습니다. 채팅방이 늘어날수록 목록 조회가 느려지는 병목을 확인했습니다.

<div class="port-decision">
목록 자체를 DB 페이지네이션으로 나누고, 마지막 메시지·읽지 않은 수·참여자 정보를 채팅방 단위로 반복 조회하지 않도록 집계 쿼리로 개선했습니다. 반복 조회가 많은 목록 데이터에는 Redis 캐시를 적용하고, 메시지 전송·읽음 상태 변경처럼 목록에 영향을 주는 이벤트가 발생하면 캐시를 갱신하도록 구성했습니다. 이를 통해 채팅방 수에 비례해 증가하던 조회 흐름을 줄이고, 실시간 기능의 데이터 변경과 목록 조회를 함께 고려했습니다.
</div>
